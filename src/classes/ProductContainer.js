...
export default class ProductContainer {
    ...
    async save(product) {
        if (!product.title) {
            ...
            return returnMessage(true, 'Ingrese Titulo', null);
        }

        if (!product.description) {
            await this.deleteImage(product.thumbnail);
            return returnMessage(true, 'Ingrese Descripcion', null);
        }

        if (!product.code) {
            await this.deleteImage(product.thumbnail);
            return returnMessage(true, 'Ingrese Codigo', null);
        }

        if (!product.stock) {
            await this.deleteImage(product.thumbnail);
            return returnMessage(true, 'Ingrese Stock', null);
        }

        if (!product.price) {
            await this.deleteImage(product.thumbnail);
            return returnMessage(true, 'Ingrese Precio', null);
        }

        if (!product.thumbnail) {
            await this.deleteImage(product.thumbnail);
            return returnMessage(true, 'Ingrese Imagen', null);
        }
        ...
        try {
            ...
            return returnMessage(true, 'Error Saving Product', null);
        }
    }
    ...
    async deleteById(id) {
        try {
            const products = (await this.getAll()).payload;
            const eliminatedProduct = products.find(
